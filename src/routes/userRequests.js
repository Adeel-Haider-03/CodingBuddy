...
userRouter.get("/user/requests/sent", userAuth, async (req, res) => {
  try {
    ...
    res.send({
      message: "Your all sent requests that are pending",
...
userRouter.get("/user/requests/friends", userAuth, async (req, res) => {
  try {
    const SAFE_DATA = [
      ...
      "gender",
      "skills",
    ]; //we will only send these fields to the client and not the whole user object (for security reasons)
    ...
    const data = friends.map((friend) => {
      ...
      if (friend.fromUserId._id.equals(loggedInUser._id)) {
        //bcoz we want only user detail of friend not us
